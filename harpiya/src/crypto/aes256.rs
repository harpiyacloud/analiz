...
pub(crate) fn encrypt(plaintext: &[u8], key: &[u8]) -> Result<Vec<u8>, ApiError>{
...
pub(crate) fn decrypt(data: &[u8], key: &[u8]) -> Result<Vec<u8>, ApiError> {
    if data.len() <= NONCE_SIZE {
        tracing::info!("invalid data length")
    }
